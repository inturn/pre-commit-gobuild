...
import (
	...
	"github.com/inturn/pre-commit-gobuild/internal/errchecker"
)
...
func reportUncheckedErrors(e *errchecker.UncheckedErrors, verbose bool) {
	wd, err := os.Getwd()
	if err != nil {
		wd = ""
	}
...
func mainCmd(args []string) int {
	...
	if err := checker.CheckPackages(paths...); err != nil {
		if e, ok := err.(*errchecker.UncheckedErrors); ok {
			reportUncheckedErrors(e, checker.Verbose)
			return exitUncheckedError
		} else if err == errchecker.ErrNoGoFiles {
...
func parseFlags(checker *errchecker.Checker, args []string) ([]string, int) {
